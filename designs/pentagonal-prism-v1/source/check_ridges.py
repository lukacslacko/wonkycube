...
sections=json.loads((CACHE/'ridges-2/sections.json').read_text());report=[];fail=[];occluded=0
for row in ROWS:
 name=row['name'];base=canonical(row['orbit']);q=mapping(base['signature'],row['signature'])
 delivered=from_mesh(printed(name));cad=load(CACHE/'final'/(name+'.npz'))
 for ridge,info in enumerate(sections[row['orbit']]):
  assert all(r['radius_mm']==2. for r in info['sections'])
  ...
  for x in np.r_[np.arange(17.25,32.01,.25),34.,36.,38.,40.,42.,44.]:
   ...
   a=lo+(hi-lo)*np.linspace(.1,.9,25);zs=cz+2*np.sin(a);expected=cy-2*np.cos(a)
   ...
   err=abs(np.hypot(new[available]-cy,zs[available]-cz)-2)
...
result=dict(manifest_sha256=hashlib.sha256((DEST/'manifest.json').read_bytes()).hexdigest(),nominal_radius_mm=2.,sections=report,occluded_or_terminated_sections=occluded,failures=fail,passed=not fail,scope='Constant R2 cutter sections through inner tracks and outer ridges; exposed-arc eligibility determined from pre-export CAD. Intersections with other fillets or exterior surfaces can terminate an arc.')
