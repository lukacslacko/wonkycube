...
REPORT={'manifest_sha256':hashlib.sha256((DEST/'manifest.json').read_bytes()).hexdigest()}
...
def motion():
 for axis,n in enumerate(AXES):
  ...
  record(f'axis_{axis}',test_path(union([PARTS[k] for k in moving]),fixed,n,np.arange(0,(180 if axis<5 else 72)+.001,3)))
 state=PARTS.copy();sigs={r['name']:tuple(r['signature']) for r in ROWS}
 for step,axis in enumerate([0,5,1,6,2,5,3,6,4,5,0,6,2,1,5,4,3,6,0,5,1,6,3,2]):
  ...
  record(f'scramble_{step}',test_path(union([state[k] for k in moving]),fixed,n,np.arange(0,(180 if axis<5 else 72)+.001,6)))
  q=Rotation.from_rotvec(np.radians(180 if axis<5 else 72)*n).as_matrix()
...
def assembly():
 rank={'K':0,'V':1,'H':1,'C':2};offsets=np.r_[np.arange(0,8.001,.25),np.arange(9,101,2)]
 ...
 for row in sorted(ROWS,key=lambda r:(rank[r['family']],r['name'])):
  ...
  if row['family']=='C':
   others += [h for i in range(7) if f'C{i+1:02}'!=name and i!=0 for h in [BOLTS[i],WASHERS[i]]]
  if name!='C01':others += [jig]+list(hardware(AXES[0],0.0))
  fixed=union(others);worst=[0.,0.]
  for d in offsets:
   v=volume(PARTS[name].translate(d*n),fixed)
   if v>worst[1]:worst=[float(d),v]
  record('insert_'+name,dict(samples=len(offsets),worst=worst,installed_hardware_included=True));assert worst[1]<.001
 others=union([s for k,s in PARTS.items() if k!='C01']+[CORE]+BOLTS[1:]+WASHERS[1:]);worst=[0.,0.]
 for d in offsets:
...
def plates():
 ...
 for row in META['orientation_options']:
  ...
  assert row['first_layer_0_16_mm_area_mm2']>0
  if 'face-down' in row['file']:assert row['first_layer_0_16_mm_area_mm2']>4
