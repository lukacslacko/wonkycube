"""Export and reload compact Redi print parts; preserve solved v4.2 IDs."""
...
def orient(s,kind,n=None):
 m=clean_mesh(mesh(s));bed_area=None
...
def add(name,s,folder,kind,n=None,group=None):
 ...
 if prior and prior['geometry_sha256']==fingerprint and path.exists() and hashlib.sha256(path.read_bytes()).hexdigest()==prior['sha256']:
  ...
  s=main(s).simplify(.002);m,T,area=orient(s,kind,n);m.export(path)
 ...
 (DEST/'manifest.json').write_text(json.dumps(dict(version='v4.3',units='millimeter',cube_side_mm=SIDE,parts=manifest),indent=2))
 print(name,len(re.faces),flush=True)

for row in ROWS:
 name=row['name'];add(name,load(OUT/'final'/(name+'.npz')),'puzzle','axis' if name[0]=='C' else 'edge',np.array(row['direction']),group=name[0])
add('core',core(),'puzzle','core',AXES[0],group='core')
for af in [5.5,5.7]:add(f'core-nut-{af:.2f}',core(af=af),'optional','core',AXES[0],group=f'optional-core-{af:.2f}')
for af in [5.5,5.6,5.7]:add(f'nut-fit-{af:.2f}',coupon(af),'fit-coupons','coupon')
add('rotor-fit',rotor_coupon(),'fit-coupons','coupon')
...
(DEST/'manifest.json').write_text(json.dumps(dict(version='v4.3',units='millimeter',cube_side_mm=SIDE,cone_half_angle_deg=float(THETA0),parts=manifest,plates=plates),indent=2))
...
page='<meta charset="utf-8"><title>Compact Redi print plates</title><style>body{font:16px system-ui;max-width:1050px;margin:40px auto}section{display:inline-block;width:48%;vertical-align:top}img{width:100%}h2{font-size:18px}</style><h1>Compact Redi v4.3 print plate maps</h1><p>Geometry-only plates: set your own PLA/support settings. Mark each part with its full ID.</p>'
