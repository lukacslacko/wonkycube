"""Wonky Skewb: tetrahedral plane cuts with integral stepped retainers (mm)."""
...
WORK=Path(os.environ.get('SKEWB_BUILD_DIR',HERE.parent))
CACHE=WORK/'cache'
SIDE=64.
BODY_GAP=.03
TRACK_EXTRA=.40
...
NUT_SEAT_AF=5.40
NUT_ENTRY_AF=5.85
SEG=int(os.environ.get('SKEWB_SEGMENTS','192'))
...
ROWS=[]
for count,letter in [(1,'C'),(2,'F'),(3,'K')]:
 for k,sig in enumerate(itertools.combinations(range(4),count)):
...
def profile():
 return [[0,0],[18.2,0],[18.2,-3.5],[22.,-3.5],[22.,3.5],[25.5,3.5],[25.5,0],[200,0],[200,200],[0,200]]
def cut(offset=0,rounding=0):
 ...
 if rounding<0:p=p.offset(rounding,join_type=mf.JoinType.Round,circular_segments=48).offset(-rounding,join_type=mf.JoinType.Round,circular_segments=48)^p
 if rounding>0:p=p+p.offset(rounding,join_type=mf.JoinType.Round,circular_segments=48).offset(-rounding,join_type=mf.JoinType.Round,circular_segments=48)
 if offset>BODY_GAP/2+.001:
  ramp=mf.CrossSection([[[26.5,-200],[200,-200],[200,-BODY_GAP/2],[29,-BODY_GAP/2],[26.5,-offset]]])
  p=p-ramp
 if offset<0:p=p+mf.CrossSection.square([1,199],False).translate([0,1])
 return p.revolve(SEG)
def raw_cells():
 inner=mf.Manifold.sphere(INNER_R,SEG);shell=mf.Manifold.cube([128]*3,True)-inner
 small=cut(-BODY_GAP/2,-.60);large=cut(BODY_GAP/2+TRACK_EXTRA,.45)
 # Extra relief is on the negative side of each inner cut. This preserves
 # the moving positive-side flange, using the unrounded rotational surface.
 # The extra track gap tapers back to the main-face gap over rho 26.5–29.
 ...
 for family in ['C','F','K']:
  ...
  s=subtract(s,[co[i] for i in range(4) if i not in sig])
  chunks=sorted(s.decompose(),key=lambda c:c.volume(),reverse=True)
  for extra in chunks[1:]:
   assert np.linalg.norm(mesh(extra).vertices,axis=1).max()<28 and extra.volume()<30
  s=chunks[0]
...
def build_relief(cells):
 result=cells.copy();limit=mf.Manifold.sphere(28,SEG)
 for family,earlier in [('F',['K']),('C',['K','F'])]:
...
def insertion_tests(cells):
 out=[]
 for r in ROWS:
  if r['name'] not in ['C01','F01','K01']:continue
  name=r['name'];n=np.array(r['direction']);rank={'K':0,'F':1,'C':2};rr=rank[r['family']]
