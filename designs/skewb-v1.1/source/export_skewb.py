...
def oriented_copy(m,kind,n=None):
 ...
 if kind in ['F','K']:q=Rotation.align_vectors([[0,0,1]],[n])[0].as_matrix()
...
for row in (r for r in ROWS if r['family'] in ['F','K']):
 ...
 m,t,area=oriented_copy(m,'face-down',row['direction'])
 path=DEST/'print-options'/(name+'-face-down.3mf');save_3mf(path,[(name,m,[128,128,0])])
 ...
 groups.setdefault(row['family']+'-face-down',[]).append((name,m))
add('core',core(),'puzzle','core',AXES[0],description='5.40 mm terminal seat, 5.85 mm loading entrance')
for af in [5.30,5.50]:add(f'core-seat-{af:.2f}',core(af),'optional','core',AXES[0],group='alternate-cores',description=f'{af:.2f} mm terminal seat; choose only one core')
for af in [5.30,5.40,5.50]:add(f'nut-fit-{af:.2f}',coupon(af),'fit-coupons','coupon',group='fit-coupons')
...
(DEST/'manifest.json').write_text(json.dumps(dict(version='wonky-skewb-v1.1',units='millimeter',cube_side_mm=SIDE,core_seat_af_mm=NUT_SEAT_AF,parts=manifest,orientation_options=orientation_options,plates=plates),indent=2))
...
(DEST/'reference/ridge-sections.json').write_text((CACHE/'ridges-3/sections.json').read_text())
page='<meta charset="utf-8"><title>Wonky Skewb print plates</title><style>body{font:16px system-ui;max-width:1050px;margin:40px auto}section{display:inline-block;width:48%;vertical-align:top}img{width:100%}h2{font-size:18px}</style><h1>Wonky Skewb — plate maps</h1><p>Millimetres, 100% scale. Geometry only. Mark every piece with its full ID.</p>'
