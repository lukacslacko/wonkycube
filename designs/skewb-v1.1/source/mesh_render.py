...
def render(objects,size=720,extent=90,elev=24,az=-55):
 ...
 light=normalize(.55*direction+.7*up-.4*right);pixels=np.full((size,size,3),[250,250,248],np.uint8);depth=np.full((size,size),-1e10,float)
 for name,T,shift,color in objects:
  ...
  shades=rgb(color)[None,:]*(.35+.45*np.maximum(norm@light,0)+.20*np.maximum(norm@direction,0))[:,None]
