...
baseline=None
if (DEST/'reference/v1-manifest.json').exists():
 baseline={r['name']:r for r in json.loads((DEST/'reference/v1-manifest.json').read_text())['parts']}
 prior={r['name']:r for r in json.loads((DEST/'reference/v1-export-fidelity.json').read_text())['parts']}
for row in META['parts']:
 name=row['name']
 if name not in [r['name'] for r in ROWS]:continue
 if baseline is not None and name.startswith(('E','K')):
  assert row['sha256']==baseline[name]['sha256']==hashlib.sha256((DEST/row['file']).read_bytes()).hexdigest()
  inherited=dict(prior[name]);inherited['unchanged_v1_stl_sha256']=row['sha256'];out.append(inherited);print(name,'byte-identical; original export audit retained',flush=True);continue
 ...
 # Absolute cleanup-volume budget, independently backed by surface samples,
 # delivered fillet measurements and collision tests. This is not a vertex bound.
 assert removed<.50 and added<.50,(name,removed,added)
 measurements=[]
...
(DEST/'reports/export-fidelity.json').write_text(json.dumps(dict(manifest_sha256=hashlib.sha256((DEST/'manifest.json').read_bytes()).hexdigest(),passed=True,parts=out,scope='Bidirectional vertex and face-center samples against the .001 mm normal-error regularized export input, not an exact Hausdorff bound or a vertex-movement bound on CAD regularization. Original-CAD versus print ridge profiles are checked separately. Byte-identical E/K STLs in the compatible patch retain their original v1 CAD-to-export audit, verified by file hash.'),indent=2))
