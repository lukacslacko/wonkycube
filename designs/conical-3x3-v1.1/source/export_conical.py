...
def oriented_copy(m,kind,n=None):
 ...
 if kind in ['E','radial-down']:q=Rotation.align_vectors([[0,0,1]],[n])[0].as_matrix()
...
for row in (r for r in ROWS if r['family'] in ['E','K']):
 ...
 option='face-down' if row['family']=='E' else 'radial-down'
...
(DEST/'manifest.json').write_text(json.dumps(dict(version='wonky-conical-3x3-70-v1.1',units='millimeter',cube_side_mm=SIDE,core_seat_af_mm=NUT_SEAT_AF,parts=manifest,orientation_options=orientation_options,plates=plates),indent=2))
...
page='<meta charset="utf-8"><title>Wonky conical 3×3 — 70° print plates</title><style>body{font:16px system-ui;max-width:1050px;margin:40px auto}section{display:inline-block;width:48%;vertical-align:top}img{width:100%}h2{font-size:18px}</style><h1>Wonky conical 3×3 — 70° — plate maps</h1><p>Millimetres, 100% scale. Geometry only. Mark every piece with its full ID.</p>'
