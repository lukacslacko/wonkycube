...
def ridges(row):
 ...
 for i,j in itertools.combinations(range(6),2):
  if abs(AXES[i]@AXES[j])>.1:continue
  k=np.cross(AXES[i],AXES[j]);k/=np.linalg.norm(k)
  for sign in [-1,1]:
   d=c*(AXES[i]+AXES[j])+sign*np.sqrt(1-2*c*c)*k
   if not all((AXES[m]@d>c)==(m in sig) for m in range(6) if m not in (i,j)):continue
...
def fronts(polygons,band=17):
 ...
 for lo,hi in zip(np.unique(breaks)[:-1],np.unique(breaks)[1:]):
  ...
  if len(active)>=2:
   # The six-axis cells have a separate outer return behind their inner foot.
