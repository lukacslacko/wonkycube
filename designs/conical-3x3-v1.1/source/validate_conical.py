...
def motion():
 for axis,n in enumerate(AXES):
  ...
  record(f'axis_{axis}',test_path(union([PARTS[k] for k in moving]),fixed,n,np.arange(0,90.001,3)))
 state=PARTS.copy();sigs={r['name']:tuple(r['signature']) for r in ROWS}
 for step,axis in enumerate([0,1,2,3,4,5,1,3,2,0,5,4,0,2,4,1,5,3,2,1,4,0,3,5]):
  ...
  record(f'scramble_{step}',test_path(union([state[k] for k in moving]),fixed,n,np.arange(0,90.001,6)))
  q=Rotation.from_rotvec(np.radians(90)*n).as_matrix()
...
def assembly():
 rank={'K':0,'E':1,'C':2};offsets=np.r_[np.arange(0,8.001,.25),np.arange(9,101,2)]
 ...
 for row in sorted(ROWS,key=lambda r:(rank[r['family']],r['name'])):
  ...
  if row['family']=='C':
   others += [h for i in range(6) if f'C{i+1:02}'!=name and i!=0 for h in [BOLTS[i],WASHERS[i]]]
  if name!='C01':others += [jig]+list(hardware(AXES[0],28.5-SEAT))
