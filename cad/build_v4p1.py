"""Build v4.1 radial-ridge replacement edges, retaining all other v4 parts."""
from mechanism_v4p1 import *
...
def main():
    ...
    base=load_parts(args.base);parts=base.copy();cut=ridge_cutter();group=cube_group()
    ...
    for i,pair in enumerate(EDGES):
        ...
        Q=next(Q for Q in group if all(min(np.linalg.norm(Q@n-v) for v in targets)<1e-7 for n in [AXES[6],AXES[7]]))
        result=round_radial_ridges(base[name],Q,cut);m=clean_mesh(mesh(result));parts[name]=from_mesh(m)
    ...
    save_3mf(out/'plates/comparison_E12.3mf',[('E12_v4p1',printed['E12'],(128,128,0))])
    ...
    meta['design']='Rotated Redi 80 — v4.1, larger radial dihedral ridge fillets'
    meta['parameters'].update(radial_ridge_round=RADIAL_RIDGE_ROUND,radial_ridge_blend_start=PRESERVE_FOOT_RADIUS,radial_ridge_full_round_start=FULL_ROUND_START)
    meta['parts']=list(records.values());meta['radial_ridge_changes']=changes
    meta['compatibility']='Only edge-piece radial ridges are cut. The v4 core, corners, tracks and hardware remain unchanged. Geometry below radius 33.5 is protected; main exterior face planes and print transforms are retained.'
